...
const Reports = () => {
  ...
  const [sort, setSort] = useState("reportId");
  const [order, setOrder] = useState("asc");

  let url = `http://localhost:5000/api/reports?sortby=${sort}&order=${order}`;
  ...
  return (
    <Content>
      <div className="flex-center">
        <Title>База данных Отчетов</Title>
        <ButtonWrapper>
          <Button onPress={() => createReport(data, "reports")}>
            Скачать xlsx
          </Button>
        </ButtonWrapper>
      </div>
      ...
      <div className="filters-block">
        ...
        <ul>
          {["reportId", "created", "format"].map((el) => (
            <li
              onClick={() => setSort(el)}
      ...
      <div className="table-wrapper">
        <table>
          <thead>
            <tr>
              {["Идентификатор", "Скан", "Создан", "Формат", "Действие"].map(
                (colName, colIdx) => (
